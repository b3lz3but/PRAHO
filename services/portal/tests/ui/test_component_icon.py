...
# ── Full list of icon names registered in _ICON_PATHS ─────────────────────────
ALL_ICON_NAMES = [
    "dashboard", "invoices", "services", "tickets", "settings", "logout",
    "check", "x", "warning", "clock", "lightning", "users", "user",
    "building", "document", "folder-open", "folder", "clipboard", "chat",
    ...
    "menu", "arrow-right", "map-pin", "edit", "exclamation-circle", "paperclip",
...
class IconTagCompletenessTests(SimpleTestCase):
    ...
    def test_all_icons_count_matches_registry(self) -> None:
        """ALL_ICON_NAMES list length must equal the number of keys in _ICON_PATHS."""
        from apps.ui.templatetags.ui_components import _ICON_PATHS  # noqa: PLC0415

        registry_keys = set(_ICON_PATHS.keys())
        test_keys = set(ALL_ICON_NAMES)
        missing = registry_keys - test_keys
        extra = test_keys - registry_keys
        self.assertSetEqual(
            missing,
            set(),
            msg=f"Icons in _ICON_PATHS not covered by ALL_ICON_NAMES: {missing}",
        )
        self.assertSetEqual(
            extra,
            set(),
            msg=f"Icons in ALL_ICON_NAMES not in _ICON_PATHS: {extra}",
        )
