...
> **Branch**: `codex/ui-ux-design-system-epic`
...
- **No breaking API changes to existing tags** — `button`, `input_field`, `checkbox_field`, `badge`, `alert`, `modal`, `table_enhanced` must remain backward-compatible.
...
| 3 | Single epic branch (`feat/ui-ux-design-system-epic`) with internal phase gates | Atomic merge; prevents partial design system in main |
...
| `{% table_enhanced %}` | `components/table_enhanced.html` | Data | ✅ Stable |
...
| `table_enhanced` | `EnhancedTableConfig` | *(none)* | *(single)* | default, empty, loading | `role="table"`, sortable headers |
| **New components** | | | | | |
...
**Shared components** (18): alert, badge, breadcrumb, button, card, checkbox, dangerous_action_modal, input, mobile_header, mobile_nav_item, modal, nav_dropdown, pagination, progress_indicator, step_navigation, table, table_enhanced, toast.
...
| 2 | 🔴 High | Zero test coverage for all 16 UI template tags | P2.10 |
