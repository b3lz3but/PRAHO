...
LIGHT_BG_CLASSES = re.compile(
    r"""\b(bg-(?:white|gray-(?:50|100|200)))\b""",
    re.IGNORECASE,
)

# Light-only text colors
LIGHT_TEXT_CLASSES = re.compile(
    r"""\b(text-(?:black|gray-(?:800|900)))\b""",
    re.IGNORECASE,
)

# Light-only border colors
LIGHT_BORDER_CLASSES = re.compile(
    r"""\b(border-(?:gray-(?:100|200|300)))\b""",
    re.IGNORECASE,
)
...
NON_TOKEN_GRAY = re.compile(
    r"""\b((?:bg|text|border|ring|divide)-gray-\d+)\b""",
    re.IGNORECASE,
)

# Dark-mode variant on same line (indicates awareness)
DARK_VARIANT = re.compile(r"\bdark:", re.IGNORECASE)
...
def _strip_comments(line: str) -> str:
    """Remove Django comments and HTML comments."""
    line = DJANGO_COMMENT.sub("", line)
    return re.sub(r"<!--.*?-->", "", line)


def _has_dark_variant_nearby(lines: list[str], line_idx: int, cls: str) -> bool:
    """Check if there's a dark: variant for this class nearby (within 2 lines).
...
def check_file(path: Path, *, verbose: bool = False) -> list[DarkModeViolation]:
    ...
    for i, raw_line in enumerate(lines, 1):
        ...
        for match in NON_TOKEN_GRAY.finditer(line):
            cls = match.group(1)
            # gray- is the non-token palette; slate- is the design token palette
            violations.append(
                DarkModeViolation(
                    code="DM005",
                    severity=SEVERITY_WARNING,
