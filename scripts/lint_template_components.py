...
_RAW_INPUT_RE = re.compile(r"<input\b(?![^>]*type=['\"]hidden['\"])", re.IGNORECASE)
...
def scan_file(path: Path) -> list[Violation]:
    ...
    for line_no, raw_line in enumerate(lines, start=1):
        ...
        if is_feature:
            ...
            color_match = _SEMANTIC_COLOR_RE.search(raw_line)
            if color_match and not _COLOR_CONTEXT_EXCLUDE.search(raw_line):
                violations.append(
                    Violation(
                        "TMPL005",
                        SEVERITY_WARNING,
